<template>
  <div class="min-h-screen bg-gray-100 pt-24">
    <div class="max-w-5xl mx-auto pt-32 px-4">
      ...
      <div
        class="max-w-sm flex flex-col items-center justify-center mx-auto"
        v-if="cart.length === 0"
      >
        ...
        <nuxt-link
          to="/coffee"
          href="#"
          class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-primary-600 hover:bg-primary-500 focus:outline-none focus:shadow-outline transition duration-150 ease-in-out md:py-4 md:text-lg md:px-10 mt-12"
        >Continue Shopping</nuxt-link>
      </div>
      ...
      <template v-if="cart.length >0">
        <div class="flex w-full justify-end items-end mt-16 flex-col">
          <h3 class="w-full text-right">
            Subtotal
            <span class="pl-12">${{checkout.subtotalPrice}} USD</span>
          </h3>
...
export default {
  ...
  computed: {
    products() {
      return this.$store.state.coffee.products;
    },
    currentCandle() {
      return this.$store.state.coffee.currentCandle;
    },
    checkout() {
      return this.$store.state.coffee.checkout;
    },
    checkoutID() {
